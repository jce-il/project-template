    ...
    "intersect": function(src, $over) {
      ...
      "resizeCell": function($cell, newCellWidth,newCellHeight,effect,updateTableFlag) {
         ...
         if(jimUtil.exists($cell) && ($cell.is(".cellcontainer") || $cell.is(".datacell") || $cell.is(".textcell") )){
           ...
           jimResponsive.resetOriginalTableSize($cell.closest("table"),jimUtil.exists(newCellWidth),jimUtil.exists(newCellHeight));
         }
      },
      ...
      "resizeTable": function($table, newTableWidth, newTableHeight, effect, callback) {
         ...
         if(jimUtil.exists($table) && ($table.is(".table") || $table.is(".datalist") || $table.is(".datagrid"))){
              if(!jimUtil.exists(newTableWidth) || isNaN(parseInt(newTableWidth, 10)))
                ...
                newTableWidth= Math.max(newTableWidth - jimEvent.fn.getCurrentStyle('padding-left', $table) - jimEvent.fn.getCurrentStyle('padding-right', $table),0);
              }
              if(newTableWidth>0){
                 ...
                 var tableBorderRightWidth = isNaN(jimEvent.fn.getCurrentStyle('border-right-width', $table)) ? 0 : jimEvent.fn.getCurrentStyle('border-right-width', $table);
              	newTableWidthNoBorders = newTableWidth - tableBorderLeftWidth - tableBorderRightWidth;
              }
              
              if(!jimUtil.exists(newTableHeight) || isNaN(parseInt(newTableHeight, 10)))
                newTableHeight = $table[0].getBoundingClientRect().height;
              else{
                newTableHeight= Math.max(newTableHeight - jimEvent.fn.getCurrentStyle('padding-top', $table) - jimEvent.fn.getCurrentStyle('padding-bottom', $table),0);
              }
              if(newTableHeight>0){
                  ...
                  newTableHeightNoBorders = newTableHeight - tableBorderTopWidth - tableBorderBottomWidth;
              }
